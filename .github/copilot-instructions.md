...
**FOLDER STRUCTURE QUICK REFERENCE (verify before every file edit):**
```
<service>-ms/
├── <service>-api/          → OpenAPI spec + generated models/interfaces (DO NOT implement logic here)
├── <service>-client/       → Client library config (ApiClient bean for other services)
└── <service>-service/      → Server implementation (Controllers, Services, Entities, Repos go HERE)
    └── src/main/java/.../app/
...
```
When asked to update "controllers" or "services" → edit files in `<service>-service/`
When asked to update "client" → edit files in `<service>-client/`
...
Microservice structure (3-module pattern):
- <service>-ms/  (aggregator/parent POM)
  - pom.xml
  - README.md
  - .env-example
  - <service>-api/  (OpenAPI spec + generated models/interfaces)
    - pom.xml (no plugin versions; depends on com.corems.common:api)
    - src/main/resources/
      - <service>-api.yaml  (OpenAPI spec)
      - .gen/  (DO NOT COMMIT - generated at build time)
    - target/generated-sources/openapi/  (generated models & API interfaces)
  - <service>-client/  (client library for other services to consume this API)
    - pom.xml
    - src/main/java/com/corems/<servicems>/client/
      - <Service>MsClientConfig.java  (Spring auto-config for ApiClient bean)
    - src/main/resources/META-INF/spring/
      - org.springframework.boot.autoconfigure.AutoConfiguration.imports
  - <service>-service/  (server implementation - controllers, services, entities, repos)
    - pom.xml
    - src/main/java/com/corems/<servicems>/app/
      - <Service>ServiceApplication.java  (main class with @EnableCoreMsLogging, @EnableCoreMsErrorHandling, @EnableCoreMsSecurity)
      - controller/  (implement generated API interfaces from *-api module)
        - *Controller.java
      - service/  (business logic)
        - *Service.java
      - repository/  (JPA repos implementing SearchableRepository)
        - *Repository.java
      - entity/  (JPA entities)
        - *.java
      - model/  (internal DTOs if needed - prefer generated models from *-api)
      - config/  (service-specific Spring configuration)
    - src/main/resources/
      - application.yaml  (server.port, spring.application.name, config imports)
      - db-config.yaml  (datasource config from env vars)
      - security-config.yaml  (JWT settings)
    - src/test/java/...  (tests)

File placement rules (CRITICAL - always verify before editing):
- Controllers/Services/Entities/Repos → ALWAYS go in `<service>-service/src/main/java/.../app/`
- Client config (ApiClient bean) → ALWAYS goes in `<service>-client/src/main/java/.../client/`
- OpenAPI spec → ALWAYS goes in `<service>-api/src/main/resources/`
- Generated models/interfaces → Generated by Maven in `<service>-api/target/generated-sources/openapi/` (do NOT manually edit)
- Application configs (yaml) → ALWAYS go in `<service>-service/src/main/resources/`
